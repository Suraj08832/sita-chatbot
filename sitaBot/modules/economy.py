import random
from datetime import datetime, timedelta
from telegram import Update, ParseMode
from telegram.ext import CallbackContext, CommandHandler, run_async
from sitaBot import dispatcher
from sitaBot.modules.helper_funcs.chat_status import user_admin
from sitaBot.modules.sql import economy_sql as sql

ITEMS = {
    "rose": {"emoji": "🌹", "price": 500, "name": "Rose"},
    "chocolate": {"emoji": "🍫", "price": 800, "name": "Chocolate"},
    "ring": {"emoji": "💍", "price": 2000, "name": "Ring"},
    "teddy": {"emoji": "🧸", "price": 1500, "name": "Teddy Bear"},
    "pizza": {"emoji": "🍕", "price": 600, "name": "Pizza"},
    "surprise": {"emoji": "🎁", "price": 2500, "name": "Surprise Box"},
    "puppy": {"emoji": "🐶", "price": 3000, "name": "Puppy"},
    "cake": {"emoji": "🎂", "price": 1000, "name": "Cake"},
    "letter": {"emoji": "💌", "price": 400, "name": "Love Letter"},
    "cat": {"emoji": "🐱", "price": 2500, "name": "Cat"},
}


@run_async
def balance(update: Update, context: CallbackContext):
    message = update.effective_message
    user = update.effective_user
    chat = update.effective_chat
    
    if message.reply_to_message:
        target_user = message.reply_to_message.from_user
        user_id = target_user.id
        name = target_user.first_name
    else:
        user_id = user.id
        name = user.first_name
    
    balance = sql.get_balance(user_id, chat.id)
    message.reply_text(f"💰 {name}'s Balance: ${balance}")


@run_async
def top_rich(update: Update, context: CallbackContext):
    users = sql.get_top_rich(10)
    
    if not users:
        update.effective_message.reply_text("📊 No economy data yet!")
        return
    
    text = "🌍 <b>Top 10 Richest Users</b>\n\n"
    for i, user_eco in enumerate(users, 1):
        medal = "🥇" if i == 1 else "🥈" if i == 2 else "🥉" if i == 3 else f"{i}."
        text += f"{medal} User {user_eco.user_id}: ${user_eco.balance}\n"
    
    update.effective_message.reply_text(text, parse_mode=ParseMode.HTML)


@run_async
def top_killers(update: Update, context: CallbackContext):
    users = sql.get_top_killers(10)
    
    if not users:
        update.effective_message.reply_text("⚔️ No kill data yet!")
        return
    
    text = "☠️ <b>Top 10 Killers</b>\n\n"
    for i, user_eco in enumerate(users, 1):
        if user_eco.kills == 0:
            continue
        medal = "🥇" if i == 1 else "🥈" if i == 2 else "🥉" if i == 3 else f"{i}."
        text += f"{medal} User {user_eco.user_id}: {user_eco.kills} kills\n"
    
    update.effective_message.reply_text(text, parse_mode=ParseMode.HTML)


@run_async
def my_rank(update: Update, context: CallbackContext):
    user = update.effective_user
    chat = update.effective_chat
    
    rank = sql.get_user_rank(user.id, chat.id)
    balance = sql.get_balance(user.id, chat.id)
    
    if rank:
        update.effective_message.reply_text(f"🏆 Your Global Rank: #{rank}\n💰 Balance: ${balance}")
    else:
        update.effective_message.reply_text("You haven't joined the economy yet!")


@run_async
def rob(update: Update, context: CallbackContext):
    message = update.effective_message
    user = update.effective_user
    chat = update.effective_chat
    
    if not message.reply_to_message:
        message.reply_text("❌ Reply to a user to rob them!")
        return
    
    target = message.reply_to_message.from_user
    
    if target.id == user.id:
        message.reply_text("❌ You can't rob yourself!")
        return
    
    if target.is_bot:
        message.reply_text("❌ You can't rob bots!")
        return
    
    if sql.is_dead(user.id, chat.id):
        message.reply_text("💀 You're dead! Use /revive to come back.")
        return
    
    if sql.is_protected(target.id, chat.id):
        message.reply_text("🛡️ This user is protected!")
        return
    
    if sql.is_dead(target.id, chat.id):
        message.reply_text("💀 They're already dead!")
        return
    
    if context.args:
        try:
            amount = int(context.args[0].replace("$", "").replace(",", ""))
        except ValueError:
            message.reply_text("❌ Invalid amount!")
            return
    else:
        message.reply_text("❌ Usage: /rob <amount> (reply to user)")
        return
    
    if amount <= 0:
        message.reply_text("❌ Amount must be positive!")
        return
    
    target_balance = sql.get_balance(target.id, chat.id)
    
    if target_balance < amount:
        message.reply_text(f"❌ They don't have ${amount}!")
        return
    
    success = random.choice([True, False])
    
    if success:
        sql.update_balance(user.id, chat.id, amount)
        sql.update_balance(target.id, chat.id, -amount)
        message.reply_text(f"🦹‍♂️ Success! You robbed ${amount} from {target.first_name}!")
    else:
        penalty = amount // 4
        robber_balance = sql.get_balance(user.id, chat.id)
        
        if robber_balance >= penalty:
            sql.update_balance(user.id, chat.id, -penalty)
            sql.update_balance(target.id, chat.id, penalty)
            message.reply_text(f"❌ Rob failed! You paid ${penalty} to {target.first_name}!")
        else:
            message.reply_text(f"❌ Rob failed! You don't have enough for the penalty.")


@run_async
def kill(update: Update, context: CallbackContext):
    message = update.effective_message
    user = update.effective_user
    chat = update.effective_chat
    
    if not message.reply_to_message:
        message.reply_text("❌ Reply to a user to kill them!")
        return
    
    target = message.reply_to_message.from_user
    
    if target.id == user.id:
        message.reply_text("❌ You can't kill yourself!")
        return
    
    if target.is_bot:
        message.reply_text("❌ You can't kill bots!")
        return
    
    if sql.is_dead(user.id, chat.id):
        message.reply_text("💀 You're dead! Use /revive to come back.")
        return
    
    if sql.is_protected(target.id, chat.id):
        message.reply_text("🛡️ This user is protected!")
        return
    
    if sql.is_dead(target.id, chat.id):
        message.reply_text("💀 They're already dead!")
        return
    
    success = random.random() < 0.6
    
    if success:
        sql.kill_user(target.id, chat.id, user.id)
        target_balance = sql.get_balance(target.id, chat.id)
        stolen = target_balance // 10
        
        # Random reward between 200-400 for successful kill
        kill_reward = random.randint(200, 400)
        sql.update_balance(user.id, chat.id, kill_reward)
        
        if stolen > 0:
            sql.update_balance(target.id, chat.id, -stolen)
            sql.update_balance(user.id, chat.id, stolen)
            message.reply_text(f"💀 You killed {target.first_name} and stole ${stolen}!\n💰 Kill Reward: +${kill_reward}\n\nThey can /revive or wait 5 hours.")
        else:
            message.reply_text(f"💀 You killed {target.first_name}!\n💰 Kill Reward: +${kill_reward}\n\nThey can /revive or wait 5 hours.")
    else:
        message.reply_text(f"❌ Kill attempt failed! {target.first_name} got away.")


@run_async
def revive(update: Update, context: CallbackContext):
    message = update.effective_message
    user = update.effective_user
    chat = update.effective_chat
    
    if message.reply_to_message:
        target = message.reply_to_message.from_user
        user_id = target.id
        name = target.first_name
    else:
        user_id = user.id
        name = user.first_name
    
    if not sql.is_dead(user_id, chat.id):
        message.reply_text(f"❤️ {name} is not dead!")
        return
    
    cost = 200
    payer_balance = sql.get_balance(user.id, chat.id)
    
    if payer_balance < cost:
        message.reply_text(f"❌ Revive costs ${cost}. You only have ${payer_balance}.")
        return
    
    sql.update_balance(user.id, chat.id, -cost)
    sql.revive_user(user_id, chat.id)
    message.reply_text(f"❤️ {name} has been revived! Cost: ${cost}")


@run_async
def protect(update: Update, context: CallbackContext):
    message = update.effective_message
    user = update.effective_user
    chat = update.effective_chat
    
    if not context.args:
        message.reply_text("❌ Usage: /protect <1d|2d>\n\n🛡️ Protection Prices:\n1 day = $100\n2 days = $500")
        return
    
    duration = context.args[0].lower()
    
    if duration == "1d":
        cost = 100
        days = 1
    elif duration == "2d":
        cost = 500
        days = 2
    else:
        message.reply_text("❌ Use: /protect 1d or /protect 2d")
        return
    
    balance = sql.get_balance(user.id, chat.id)
    
    if balance < cost:
        message.reply_text(f"❌ Protection costs ${cost}. You only have ${balance}.")
        return
    
    sql.update_balance(user.id, chat.id, -cost)
    sql.protect_user(user.id, chat.id, days)
    message.reply_text(f"🛡️ You are protected for {days} day(s)! Cost: ${cost}")


@run_async
def give_money(update: Update, context: CallbackContext):
    message = update.effective_message
    user = update.effective_user
    chat = update.effective_chat
    
    if not message.reply_to_message:
        message.reply_text("❌ Reply to a user to give them money!")
        return
    
    target = message.reply_to_message.from_user
    
    if target.id == user.id:
        message.reply_text("❌ You can't give money to yourself!")
        return
    
    if not context.args:
        message.reply_text("❌ Usage: /give <amount> (reply to user)\n\n⚠️ Note: 10% tax will be deducted!")
        return
    
    try:
        amount = int(context.args[0].replace("$", "").replace(",", ""))
    except ValueError:
        message.reply_text("❌ Invalid amount!")
        return
    
    if amount <= 0:
        message.reply_text("❌ Amount must be positive!")
        return
    
    tax = amount // 10
    total_cost = amount + tax
    
    balance = sql.get_balance(user.id, chat.id)
    
    if balance < total_cost:
        message.reply_text(f"❌ You need ${total_cost} (${amount} + ${tax} tax). You only have ${balance}.")
        return
    
    sql.update_balance(user.id, chat.id, -total_cost)
    sql.update_balance(target.id, chat.id, amount)
    message.reply_text(f"🎁 You gave ${amount} to {target.first_name}!\n💸 Tax paid: ${tax}")


@run_async
def economy_help(update: Update, context: CallbackContext):
    text = """💰 <b>Sita's Economy System</b>

Earn virtual money by robbing, killing and playing lottery game. Most commands work only in groups.

⚡ All users will get $50 after every 12 hours!

📌 <b>Commands:</b>

/bal — Your/your friend's balance 💵
/toprich — Top 10 richest globally 🌍
/rob (reply) &lt;amount&gt; — Rob money 🦹‍♂️
/kill (reply) — Kill someone 💀
/protect &lt;1d|2d&gt; — Protect yourself 🛡️
/revive (reply or no reply) — Revive yourself or a friend ❤️
/give (reply) &lt;amount&gt; — Give money 🎁
/myrank — Show global rank 🏆
/topkill — Top 10 killers ☠️

🔒 <b>Protection Rules:</b>

Protected users cannot be robbed or killed.
Dead users auto-revive after 5 hours, or use /revive to come back sooner.

1-day protection = $100 💰 | 2-day = $500 💰💰
Revive cost = $200 💰

⚠️ Note: This is all virtual fun, no real money is involved! 🎉"""
    
    update.effective_message.reply_text(text, parse_mode=ParseMode.HTML)


@run_async
def items_list(update: Update, context: CallbackContext):
    text = "🛍️ <b>Available Items</b>\n\n"
    
    for key, item in ITEMS.items():
        text += f"{item['emoji']} {item['name']} — ${item['price']}\n"
    
    text += "\n💡 Use /gift &lt;item&gt; to gift items to friends!"
    
    update.effective_message.reply_text(text, parse_mode=ParseMode.HTML)


@run_async
def inventory(update: Update, context: CallbackContext):
    message = update.effective_message
    user = update.effective_user
    
    if message.reply_to_message:
        target_user = message.reply_to_message.from_user
        user_id = target_user.id
        name = target_user.first_name
    else:
        user_id = user.id
        name = "Your"
    
    items = sql.get_inventory(user_id)
    
    if not items:
        message.reply_text(f"📦 {name} inventory is empty!")
        return
    
    text = f"📦 <b>{name} Inventory</b>\n\n"
    for item in items:
        for key, item_data in ITEMS.items():
            if item_data['name'].lower() == item.item_name.lower():
                text += f"{item_data['emoji']} {item.item_name} x{item.quantity}\n"
                break
        else:
            text += f"• {item.item_name} x{item.quantity}\n"
    
    message.reply_text(text, parse_mode=ParseMode.HTML)


@run_async
def gift_item(update: Update, context: CallbackContext):
    message = update.effective_message
    user = update.effective_user
    chat = update.effective_chat
    
    if not message.reply_to_message:
        message.reply_text("❌ Reply to a user to gift them an item!")
        return
    
    target = message.reply_to_message.from_user
    
    if target.id == user.id:
        message.reply_text("❌ You can't gift yourself!")
        return
    
    if not context.args:
        message.reply_text("❌ Usage: /gift <item name> (reply to user)\n\nUse /items to see available items.")
        return
    
    item_search = " ".join(context.args).lower()
    
    item_key = None
    for key, item_data in ITEMS.items():
        if item_search in item_data['name'].lower() or item_search in key:
            item_key = key
            break
    
    if not item_key:
        message.reply_text("❌ Item not found! Use /items to see available items.")
        return
    
    item = ITEMS[item_key]
    
    balance = sql.get_balance(user.id, chat.id)
    if balance < item['price']:
        message.reply_text(f"❌ {item['emoji']} {item['name']} costs ${item['price']}. You only have ${balance}.")
        return
    
    sql.update_balance(user.id, chat.id, -item['price'])
    sql.add_item(target.id, item['name'], 1)
    
    message.reply_text(f"🎁 You gifted {item['emoji']} {item['name']} to {target.first_name}!\n💰 Cost: ${item['price']}")


@run_async
def new_lottery(update: Update, context: CallbackContext):
    active = sql.get_active_lottery()
    
    if active:
        time_left = active.end_time - datetime.utcnow()
        hours = int(time_left.total_seconds() // 3600)
        update.effective_message.reply_text(f"❌ A lottery is already running! Time left: {hours}h")
        return
    
    lottery_id = sql.create_lottery()
    
    if lottery_id:
        update.effective_message.reply_text(
            "🎰 <b>New Lottery Started!</b>\n\n"
            "Duration: 2 days\n"
            "Minimum ticket: $1000\n\n"
            "Use /buy &lt;amount&gt; to join!\n"
            "Use /status to check progress",
            parse_mode=ParseMode.HTML
        )
    else:
        update.effective_message.reply_text("❌ Failed to create lottery!")


@run_async
def buy_ticket(update: Update, context: CallbackContext):
    message = update.effective_message
    user = update.effective_user
    chat = update.effective_chat
    
    lottery = sql.get_active_lottery()
    
    if not lottery:
        message.reply_text("❌ No active lottery! Use /new to start one.")
        return
    
    if not context.args:
        message.reply_text("❌ Usage: /buy <amount>\n\nMinimum: $1000")
        return
    
    try:
        amount = int(context.args[0].replace("$", "").replace(",", ""))
    except ValueError:
        message.reply_text("❌ Invalid amount!")
        return
    
    if amount < 1000:
        message.reply_text("❌ Minimum ticket is $1000!")
        return
    
    balance = sql.get_balance(user.id, chat.id)
    
    if balance < amount:
        message.reply_text(f"❌ You need ${amount}. You only have ${balance}.")
        return
    
    if not sql.join_lottery(lottery.lottery_id, user.id, amount):
        message.reply_text("❌ You already joined this lottery!")
        return
    
    sql.update_balance(user.id, chat.id, -amount)
    message.reply_text(f"🎰 You joined the lottery with ${amount}!\n\nUse /status to check progress.")


@run_async
def lottery_status(update: Update, context: CallbackContext):
    lottery = sql.get_active_lottery()
    
    if not lottery:
        update.effective_message.reply_text("❌ No active lottery!")
        return
    
    participants = sql.get_lottery_participants(lottery.lottery_id)
    
    time_left = lottery.end_time - datetime.utcnow()
    hours = int(time_left.total_seconds() // 3600)
    
    text = f"""🎰 <b>Lottery Status</b>

💰 Total Jackpot: ${lottery.total_pool}
👥 Participants: {len(participants)}
⏰ Time Left: {hours} hours

🏆 <b>Prizes:</b>
🥇 1st Place → ${lottery.total_pool * 10}
🥈 2nd Place → ${lottery.total_pool * 5}
🥉 3rd Place → ${lottery.total_pool * 3}

⚠️ Minimum 3 participants required!"""
    
    update.effective_message.reply_text(text, parse_mode=ParseMode.HTML)


@run_async
def lottery_game(update: Update, context: CallbackContext):
    text = """🎰 <b>Lottery Game Info</b>

⚙️ <b>How It Works</b>

A new lottery runs for 2 days.
Use /buy &lt;amount&gt; to join the lottery (minimum ticket = $1000).
At the end, 3 winners are randomly selected.

🏆 <b>Prizes</b>

🥇 1st Place → amount × 10
🥈 2nd Place → amount × 5
🥉 3rd Place → amount × 3

📌 <b>Commands</b>

/new → Start a new lottery (only if none running)
/buy &lt;amount&gt; → Join the active lottery
/status → Check players, jackpot & time left
/game → Game info & rules

⚠️ <b>Rules</b>

• Minimum 3 participants required, otherwise all money is refunded
• Each player can join only once per lottery"""
    
    update.effective_message.reply_text(text, parse_mode=ParseMode.HTML)


@run_async
def daily(update: Update, context: CallbackContext):
    user = update.effective_user
    chat = update.effective_chat
    
    if not sql.can_claim_daily(user.id):
        update.effective_message.reply_text("❌ You already claimed your daily bonus! Wait 12 hours.")
        return
    
    balance_before = sql.get_balance(user.id, chat.id)
    is_new_user = (balance_before == 0)
    
    if is_new_user:
        sql.update_balance(user.id, chat.id, 100)
        update.effective_message.reply_text("⚡ Daily bonus claimed! +$50\n🎉 Welcome bonus! +$50\n\n💰 Total received: $100")
    else:
        sql.update_balance(user.id, chat.id, 50)
        update.effective_message.reply_text("⚡ Daily bonus claimed! +$50")
    
    sql.update_daily(user.id)


dispatcher.add_handler(CommandHandler("bal", balance))
dispatcher.add_handler(CommandHandler("balance", balance))
dispatcher.add_handler(CommandHandler("toprich", top_rich))
dispatcher.add_handler(CommandHandler("topkill", top_killers))
dispatcher.add_handler(CommandHandler("myrank", my_rank))
dispatcher.add_handler(CommandHandler("rob", rob))
dispatcher.add_handler(CommandHandler("kill", kill))
dispatcher.add_handler(CommandHandler("revive", revive))
dispatcher.add_handler(CommandHandler("protect", protect))
dispatcher.add_handler(CommandHandler("give", give_money))
dispatcher.add_handler(CommandHandler("economy", economy_help))
dispatcher.add_handler(CommandHandler("items", items_list))
dispatcher.add_handler(CommandHandler("item", inventory))
dispatcher.add_handler(CommandHandler("inventory", inventory))
dispatcher.add_handler(CommandHandler("gift", gift_item))
dispatcher.add_handler(CommandHandler("new", new_lottery))
dispatcher.add_handler(CommandHandler("buy", buy_ticket))
dispatcher.add_handler(CommandHandler("status", lottery_status))
dispatcher.add_handler(CommandHandler("game", lottery_game))
dispatcher.add_handler(CommandHandler("daily", daily))

__mod_name__ = "Economy"
__help__ = """```
❖ єᴄσησϻʏ sʏsᴛєϻ ❖```

💰 Economy System - Earn, rob, kill, and play lottery!

Commands:
 • `/bal` - Check balance
 • `/toprich` - Top 10 richest
 • `/topkill` - Top 10 killers
 • `/myrank` - Your global rank
 • `/rob` <amount> - Rob someone (reply)
 • `/kill` - Kill someone (reply)
 • `/revive` - Revive yourself or someone (reply)
 • `/protect` <1d|2d> - Buy protection
 • `/give` <amount> - Give money (reply, 10% tax)
 • `/economy` - Full economy guide
 • `/items` - See all items
 • `/item` - Check inventory
 • `/gift` <item> - Gift an item (reply)
 • `/new` - Start new lottery
 • `/buy` <amount> - Join lottery
 • `/status` - Lottery status
 • `/game` - Lottery game info
 • `/daily` - Claim daily bonus ($50)
"""
